...
int main() {
    ...
    std::cout << obj1.operator->() << std::endl;

    {
        ...
        std::cout << obj2.operator->() << std::endl;
    ...
    std::cout << obj3.operator->() << std::endl;

    // obj1 and obj3 are released back to the pool when the program ends
    Pool<int> intPool;
    intPool.resize(5);
    
    Pool<int>::Object intObj = intPool.acquire(50);
    std::cout << *intObj.operator->() << std::endl;
    if (*intObj.operator->() != 50)
        return 1;
    return 0;
}
